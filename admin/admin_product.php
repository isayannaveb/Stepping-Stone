...
<body>
            ...
            <form method="post" enctype="multipart/form-data">
                <center>
                    <table>
                        ...
                        <tr>
                            <td><input type="hidden" name="product_code" value="<?php echo $code; ?>" required></td>
                        </tr>
                        <tr>
    ...
    <div id="rightcontent" style="position:absolute; top:10%;">
        ...
        <div class="alert alert-info">
            <table class="table table-hover" style="background-color:;">
                ...
                <tbody>
                    <?php
                        $query = $conn->query("SELECT * FROM product WHERE category='basketball' ORDER BY product_id DESC") or die(mysqli_error($conn));
                    ...
                    <tr class="del<?php echo $id ?>">
                        ...
                        <td>
                            <?php
                                $query1 = $conn->query("SELECT * FROM stock WHERE product_id='$id'") or die(mysqli_error($conn));
                                $fetch1 = $query1->fetch_array();
                                echo $fetch1['qty'];
                            ?>
                        </td>
                        <td>
                            <a href="stockin.php?id=<?php echo $id ?>" class="btn btn-success" rel="facebox"><i class="icon-plus-sign icon-white"></i> Stock In</a>
                            <a href="stockout.php?id=<?php echo $id ?>" class="btn btn-danger" rel="facebox"><i class="icon-minus-sign icon-white"></i> Stock Out</a>
                        </td>
                        <td>
    ...
    <?php
        if (isset($_POST['stockin'])) {
            $pid = $_POST['pid'];
            $result = $conn->query("SELECT * FROM stock WHERE product_id='$pid'") or die(mysqli_error($conn));
            $row = $result->fetch_array();
            $old_stck = $row['qty'];
            $new_stck = $_POST['new_stck'];
            $total = $old_stck + $new_stck;
            $que = $conn->query("UPDATE stock SET qty='$total' WHERE product_id='$pid'") or die(mysqli_error($conn));
            echo "<script>window.location = 'admin_product.php'</script>";
        }
